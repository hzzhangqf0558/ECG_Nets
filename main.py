...
import models
from dataset import HFDataset, transform
from data_preparing import name2index
from utils import loss, AverageMeter
from utils import calc_f1,calc_acc, LRScheduler
...
parser.add_argument('--config', default='configs/Top1Net.yaml')
...
def main():
    ...
    if not args.evaluate and not args.val:
        ...
        # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=niters, gamma=0.1)
        ...
        if args.loss == 'weighted_binary_crossentropy':
            ...
            # loss = nn.BCELoss(weight=weight, size_average=False, reduce=False)
        else:
    ...
    for epoch in range(opt.epochs):
        ...
        for i,(datas,ages,sexs,labels) in enumerate(train_loader):
            ...
            if (i + 1) % print_freq == 0:
                ...
                print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, val_acc: {:.4f},LR:{:.5f}, Time[{:.7s}/{:.7s}]'
                      .format(epoch + 1, opt.epochs, i + 1, total_step, loss.item(),val(val_loader, model, device),
        ...
        if not fast_train:
            ...
            # acc_train = val(train_loader, model, device)
            # print('Train Accuracy: {} %'.format(acc_train), flush=True)
            # writer.add_scalar('train_acc', acc_train, iter_used)
            # # acc in validation set
            # valacc= val_acc(val_loader, model, device)
            # print('Validation Accuracy: {} %'.format(valacc), flush=True)
            # writer.add_scalar('val_acc', valacc, iter_used)
